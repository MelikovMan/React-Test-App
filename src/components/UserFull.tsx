...
export default function UserFullInfo({id,children}:UserFullInfoProps){
    ...
    return(
        <div>
            <Container sx={{p:0}}>
            <ToolbarFullProfile first_name={user?.data.first_name ?? "Test"} last_name={user?.data.last_name ?? "Test"} avatar={user?.data.avatar ?? "Test"}/>
            <Grid   container
              justifyContent="flex-start"
             alignItems="center"
             columns={{ xs: 2, sm: 8, md: 8 }}
             spacing={{ xs: 2, md: 3 }}
             sx={{margin:"auto", boxSizing:"content-box"}}>
                ...
                <Grid item xs={2} sm={4} md={3} order={{xs:1,sm:2}}>
